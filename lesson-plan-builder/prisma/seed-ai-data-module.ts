/**
 * AI Settings Data Seeding Module
 * 
 * This module version can be imported and called programmatically
 * Used by the API for seeding data
 */
...
/** sync provider และ mappings ที่ยังไม่มีใน DB (non-destructive) + อัปเดต priority ตาม seed */
export async function syncMissingProviders(): Promise<number> {
  let synced = 0;

  for (const provider of aiProviders) {
    const existing = await prisma.aiProvider.findUnique({ where: { key: provider.key } });
    if (existing) {
      if (existing.priority !== provider.priority) {
        await prisma.aiProvider.update({
          where: { key: provider.key },
          data: { priority: provider.priority }
        });
        console.log(`🔄 Updated provider priority: ${provider.key} → ${provider.priority}`);
        synced++;
      }
      continue;
    }
    ...
    await prisma.aiProvider.create({
      data: {
        ...
        settings: (provider.settings || {}) as object,
        apiKeyEnc: encryptedKey,
        isEnabled: !!envKey || local || provider.isEnabled,
        isDefault: false,
        priority: provider.priority
      }
    });

    console.log(`🔄 Synced provider: ${provider.key}`);
    synced++;
  }
  ...
  for (const [funcKey, mappings] of Object.entries(functionProviderMappings)) {
    ...
    for (const mapping of mappings) {
      ...
      const existingMapping = await prisma.aiFunctionProvider.findUnique({
        where: {
          functionId_providerId: { functionId, providerId }
        }
      });
      ...
      if (existingMapping) {
        if (
          ...
          JSON.stringify(existingMapping.config) !== JSON.stringify(mapping.config || {})
        ) {
          await prisma.aiFunctionProvider.update({
      ...
      await prisma.aiFunctionProvider.create({
        data: {
          functionId,
          providerId,
          priority: mapping.priority,
  ...
  if (synced > 0) {
    console.log(`✅ Synced ${synced} provider/mapping item(s)`);
  }

  return synced;
}

/** @deprecated ใช้ syncMissingProviders แทน */
export async function ensureOllamaProvider(): Promise<boolean> {
  return (await syncMissingProviders()) > 0;
}
...
export async function seedAiData(): Promise<{
  success: boolean;
  message: string;
  stats: {
    functions: number;
  ...
  try {
    ...
    return {
      success: true,
      message: `Seeded ${funcCount} functions, ${providerCount} providers, ${mappingCount} mappings`,
      stats: {
        functions: funcCount,
    ...
    return {
      ...
      stats: { functions: 0, providers: 0, mappings: 0 }
    };
  }
}
