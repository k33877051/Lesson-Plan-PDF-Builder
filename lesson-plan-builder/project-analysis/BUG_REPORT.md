...
- ~~`PROCESSING` extraction status can remain stuck if the server crashes during PDF extraction.~~ **Mitigated:** API/UI allow retry after 5 minutes stale `PROCESSING`.
...
- `/dashboard/lesson-builder` exists but is now linked from sidebar (**สร้างด้วย AI**) and project detail CTA.
...
## Data Quality Issues
- ~~Older PDF extraction could store binary-looking garbage text as `COMPLETED`.~~ **Mitigated:** `isUsableExtractedText()` + pdf-parse v2 text-layer path.
...
2. ~~Finish and harden OCR/PDF extraction including stuck `PROCESSING` recovery.~~ **Done (June 2026):** pdf-parse v2, canvas 0.1.100, stale PROCESSING retry.
